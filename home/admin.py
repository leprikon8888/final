...
from .models import Service, ServiceBlock, AboutBlock, AboutUnit, Team, Client, Shapka, BrandName, BrandFace, Portfolio, \
    PortfolioBlock, ContactBlock, FormContact, Niz


@admin.register(Niz)
class NizAdmin(admin.ModelAdmin):
    list_display = ('id', 'name_brand')


@admin.register(FormContact)
class FormContactAdmin(admin.ModelAdmin):
    list_display = ('id', 'name', 'email', 'phone', 'message', 'is_confirmed')
    list_editable = ('name', 'email', 'phone', 'message', 'is_confirmed')
